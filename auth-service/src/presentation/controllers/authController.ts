...
import {
  clearCookie,
  setAuthCookie,
} from "../../infrastructure/utils/tokenHandler";
import { REGISTRATION } from "../../constants/messages";
import { AuthRequest } from "../middlewares/tokenValidation";
...
export class AuthController {
  ...
  async createUser(req: Request, res: Response, next: NextFunction) {
    ...
    return res.status(response.status ? ENUM.CREATED : ENUM.BAD_REQUEST).json({
  ...
  async loginUser(req: Request, res: Response, next: NextFunction) {
    ...
    return res.status(response.status ? ENUM.OK : ENUM.UNAUTHORIZED).json({
      status: response?.status,
      user: response?.user,
      message: response?.message,
    });
